...
def test_quality_view_recompute_not_stuck_disabled_during_export(qtbot):
    """
    Regression test: _trigger_auto_recompute() disables the tab and
    calls evaluate(blocking=False) expecting on_evaluation_finished()
    to re-enable it once done. But evaluate() returns immediately,
    before wiring that connection, whenever main_window.export_running
    is True - which used to leave the tab stuck disabled forever, with
    no way to retry (on_evaluation_deferred() must undo the disable/
    pending state itself in that case).
    """
    window = BMicro()
    qtbot.add_widget(window)
    ...
    window.widget_quality_view._trigger_auto_recompute()

    assert window.widget_quality_view._recompute_pending is False
    assert window.widget_quality_view.isEnabled()
