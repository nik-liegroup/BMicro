...
class QualityView(QtWidgets.QWidget):
    ...
    def current_peak_index(self):
        """
        Which Brillouin peak fit's diagnostics this tab should check
        and show, mirroring whatever peak the Evaluation tab's own
        combobox_peak_number is currently displaying (see
        EvaluationView.setNrBrillouinPeaks() for what each index
        means). Without this, a multi-peak fit's Quality tab would
        always check/preview index 0 - the always-computed single-peak
        reference fit - rather than the multi-peak result the user is
        actually looking at. Returns None (bmlab's own default: the
        single-peak fit, or the weighted multi-peak average once
        nr_brillouin_peaks > 1) if the Evaluation tab isn't linked yet.
        """
        if self._evaluation_view is None:
            return None
        combobox = getattr(
            self._evaluation_view, 'combobox_peak_number', None)
        if combobox is None or combobox.currentIndex() < 0:
            return None
    ...
    def _trigger_auto_recompute(self):
        ...
        self._evaluation_view.evaluate(blocking=False, notify_blocked=False)
    ...
    def on_evaluation_deferred(self):
        """
        Called by EvaluationView.evaluate() when it declines to run at
        all (e.g. a background export is in progress) - undoes
        _trigger_auto_recompute()'s own disable/pending state so this
        tab isn't left permanently stuck disabled (on_evaluation_finished(),
        which would normally do that, is never wired up in this case,
        since evaluate() returns before reaching the code that wires it).

        Deliberately does NOT call update_ui() itself: that would
        immediately retrigger _trigger_auto_recompute() while the same
        blocking condition still holds, recursing straight back here.
        A later, genuine update_ui() call - MainWindow.on_export_finished()
        once the export actually completes, or the user simply
        revisiting this tab - retries once evaluate() can actually run.
        """
        self._recompute_pending = False
        self.setEnabled(True)
    ...
    def refresh_plot(self):
        ...
        data, positions, dimensionality, labels = \
            self.evaluation_controller.get_data(
                key, self.current_peak_index())
